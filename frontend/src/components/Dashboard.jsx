...
function Dashboard({ records, filterOptions }) {
  ...
  const D = useMemo(() => {
    ...
    return {total,totalPers,totalTeams,uniqueLines,uniqueLocs,uniqueSups,
            dProg,dVolt,hbarSup,hbarWork,hbarLine,barLoc,
            lineTrend,lineMonthly,stacked,grouped,avgPT,
            svLabels:sv.map(e=>e[0]), ptLabels:pt.map(e=>e[0]),
            vlLabels:vl.map(e=>e[0]), lnLabels:ln.map(e=>e[0]),
            locLabels:loc.map(e=>e[0]), wdLabels:wd.map(e=>e[0]),
  ...
  const hbarOpts = (field, labels, showLeg=false) => ({
    ...
    plugins:{legend:{display:showLeg}, tooltip:TT},
    onClick:(_,els) => onChartClick(field,els,labels),
    scales:{
    ...
    <div className="db-root">
      ...
      {D.total > 0 ? (
        <div className="db-charts-grid">
          ...
          {/* ردیف G: میانگین به تفکیک نوع برنامه — کامل */}
