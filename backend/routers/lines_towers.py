...
from models import Line, Tower, MaintenanceRecord, PlannedTask, PowerLineRecord
...
def jalali_to_gregorian(date_str: str) -> Optional[datetime]:
    ...
    try: y, m, d = map(int, date_str.split('/'))
    except: return None
    days = (y - 1) * 365 + ((y - 1) // 4)
    if m <= 7: days += (m - 1) * 31
    else: days += 186 + (m - 7) * 30
    days += d - 1
...
def update_tower_dates(db, tower):
    ...
    if recs:
        ...
        if recs[0].gregorian_date:
            tower.next_maintenance = gregorian_to_jalali(recs[0].gregorian_date + timedelta(days=365))
    else:
...
def create_line(line: LineSchema, db: Session = Depends(get_db)):
...
def get_towers(line_id: Optional[str] = None, db: Session = Depends(get_db)):
    q = db.query(Tower)
    if line_id: q = q.filter(Tower.line_id == line_id)
    return q.all()

@router.post("/towers", response_model=TowerSchema)
def create_tower(tower: TowerSchema, db: Session = Depends(get_db)):
...
def get_maintenance_records(tower_id: Optional[str] = None, db: Session = Depends(get_db)):
    q = db.query(MaintenanceRecord)
    if tower_id: q = q.filter(MaintenanceRecord.tower_id == tower_id)
    return q.all()

@router.post("/maintenance-records", response_model=MaintenanceRecordSchema)
def create_maintenance_record(rec: MaintenanceRecordSchema, db: Session = Depends(get_db)):
    ...
    if tower_id:
        ...
        for plan in open_plans:
            if plan.gregorian_date and abs((plan.gregorian_date - (rec.gregorian_date or datetime.now())).days) <= 30:
...
def get_planned_tasks(line_id: Optional[str] = None, db: Session = Depends(get_db)):
    q = db.query(PlannedTask)
    if line_id: q = q.filter(PlannedTask.line_id == line_id)
    return q.all()

@router.post("/planned-tasks", response_model=PlannedTaskSchema)
def create_planned_task(task: PlannedTaskSchema, db: Session = Depends(get_db)):
...
def complete_planned_task(task_id: str, db: Session = Depends(get_db)):
...
def get_open_plans_for_line(line_id: str, db: Session = Depends(get_db)):
    ...
    grouped = {}
    for plan in plans:
        key = (plan.type or "", plan.description or "", plan.supervisor or "", plan.crew or "", plan.personnel or "")
        ...
        # اضافه کردن دکل با plan_id منحصربه‌فرد
        tower_entry = {
            "tower_id": plan.tower_id,
            "number": plan.number,
            "plan_id": plan.id
        }
        # جلوگیری از تکرار دکل با همان plan_id
        if not any(t["tower_id"] == plan.tower_id and t["plan_id"] == plan.id for t in grouped[key]["towers"]):
...
def import_from_records(db: Session = Depends(get_db)):
    records = db.query(PowerLineRecord).all()
    added = 0
    for rec in records:
        if not rec.line_name: continue
        line = get_or_create_line(db, rec.line_name, int(rec.voltage_level) if rec.voltage_level else 0)
        tower_nums = set()
        for f in ['tower_number', 'tower_number2']:
            ...
            if val:
                for part in str(val).replace(';',',').replace('،',',').split(','):
                    part = part.strip()
                    if part.isdigit(): tower_nums.add(int(part))
...
def complete_plans(plan_ids: List[str], db: Session = Depends(get_db)):
    for pid in plan_ids:
...
def layout_towers(db):
    ...
    paths = [
        [60,60,840,490],[60,490,840,60],[60,275,840,275],
        [450,60,450,490],[60,160,840,390],[160,60,740,490]
    ]
    for idx, line in enumerate(lines):
        towers = db.query(Tower).filter(Tower.line_id == line.id).order_by(Tower.number).all()
...
def tower_stats(db: Session = Depends(get_db)):
    towers = db.query(Tower).all()
    total = len(towers)
    with_next = sum(1 for t in towers if t.next_maintenance)
    urgent = sum(1 for t in towers if t.next_maintenance and jalali_to_gregorian(t.next_maintenance) and (jalali_to_gregorian(t.next_maintenance) - datetime.now()).days <= 30)
    return {"total_towers": total, "have_next_maintenance": with_next, "urgent": urgent}

@router.post("/update-all-tower-dates")
def update_all_tower_dates(db: Session = Depends(get_db)):
    towers = db.query(Tower).all()
    for t in towers:
        update_tower_dates(db, t)
    db.commit()
...
def mark_tower_completed(tower_id: str, db: Session = Depends(get_db)):
    tower = db.query(Tower).filter(Tower.id == tower_id).first()
    if not tower:
